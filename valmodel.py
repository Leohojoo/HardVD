...
train_hps = {  # 设置默认参数值
    ...
    'batch_size': 4,  # vuldeepecker 2:64059
    'validate_every': 5464,
# validates on small subset of val set       ours  2:105929  vulcnn 2:15551 vulcnn2 2:15986 devign 2:10927
    'evaluate_every': 5464,  # evaluates on full test set using best ckpt #d2a 4:1161 2:2322   reveal 4:4547 2:9094
...
def evaluate(dataloader, vision_model, reprogrammer, tb_writer,
    ...
    for bidx, batch in enumerate(dataloader):
        ...
        if reprogrammer.base_image is not None:
            ...
            pert_unnormalized = unnormalize_image(programmed_img, img_mean, img_std) - unnormalize_image(
                base_image_batch, img_mean, img_std)
...
def main():
    ...
    # else:
    #     tokenizer = AutoTokenizer.from_pretrained('bert-base-cased')#Tokens2Vec

    # train_dataset_raw = datasets.load_dataset(path='csv',data_files="./pytorch-image-models/d2a/function/d2a_train.csv",split=train_split)
    ...
    # # print(train_dataset.data)
    # # print(train_dataset.data[1][25])
    # print(np.size(train_dataset.data[0]))
    # print(np.size(train_dataset.data[1]))
    # print(np.size(train_dataset.data[2][0]))
    # #设置验证数据集
    # #val_dataset_raw = datasets.load_dataset(dataset_name, subset, data_files=data_files, split=val_split, cache_dir = args.cache_dir)
    # val_dataset_raw = datasets.load_dataset(path='csv',data_files="./pytorch-image-models/d2a/function/d2a_dev.csv",split=train_split)
    ...
    # #ours
    # code_train = pd.read_csv('./pytorch-image-models/ours/our_train.csv')
    # text_train = pd.read_csv('./pytorch-image-models/ours/code_train_text.csv')
    # code_val = pd.read_csv('./pytorch-image-models/ours/our_val.csv')
    # text_val = pd.read_csv('./pytorch-image-models/ours/code_val_text.csv')

    # #vulcnn
    # code_train = pd.read_csv('./pytorch-image-models/VulCNN/vulcnn_train2.csv')
    # text_train = pd.read_csv('./pytorch-image-models/VulCNN/vulcnn_train_text2.csv')
    # code_val = pd.read_csv('./pytorch-image-models/VulCNN/vulcnn_val2.csv')
    # text_val = pd.read_csv('./pytorch-image-models/VulCNN/vulcnn_val_text2.csv')
    ...
    m_state_dict = torch.load('./system/model/D2A_all_shot_ACC64.70_best_model.p')
    reprogrammer.load_state_dict(m_state_dict["state_dict"])#读取参数
    ...
    image_net_labels=m_state_dict["image_net_labels"]
    class_mapping=m_state_dict["class_mapping"]
    print("Evaluating")
    ...
    #             save_checkpoint(reprogrammer, train_hps['lr'], metrics['acc'], iter_no, image_net_labels, class_mapping,
    #                             model_path)
    #             if metrics['acc'] >= best_acc:
    #                 best_model_path = os.path.join(ckptdir, "model_best.p")
    #                 save_checkpoint(reprogrammer, train_hps['lr'], metrics['acc'], iter_no, image_net_labels,
    #                                 class_mapping, best_model_path)
    #                 best_acc = metrics['acc']
    #                 best_f1 = metrics['f1']
    #                 best_rec = metrics['rec']
    #                 best_prec = metrics['prec']
